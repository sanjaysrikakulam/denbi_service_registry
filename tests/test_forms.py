...
class TestSubmissionFormRequired:
    ...
    @pytest.mark.parametrize("field", [
        "service_name", "service_description", "submitter_first_name", "submitter_last_name", "submitter_affiliation",
        "host_institute", "public_contact_email", "internal_contact_name",
        "internal_contact_email", "internal_contact_email_confirm",
        "website_url", "terms_of_use_url", "publications_pmids",
    ])
...
class TestSubmissionFormURLValidation:
    ...
    def test_github_url_must_be_github(self):
        from apps.submissions.forms import SubmissionForm
        form = SubmissionForm(_base_form_data({"github_url": "https://gitlab.com/org/repo"}))
        assert not form.is_valid()
        assert "github_url" in form.errors

    def test_biotools_url_must_be_biotools(self):
        from apps.submissions.forms import SubmissionForm
        form = SubmissionForm(_base_form_data({"biotools_url": "https://bioinformatics.tools/x"}))
        assert not form.is_valid()
