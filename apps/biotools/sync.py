...
def sync_tool(biotools_id: str, submission_id: str | None = None) -> SyncResult:
    ...
    try:
        ...
        return SyncResult(ok=False, biotools_id=biotools_id, created=False, error=error_msg)
    ...
    if record is None:
        ...
        created = True
        logger.info("Created BioToolsRecord for %s (submission %s)", biotools_id, submission_id)
    else:
