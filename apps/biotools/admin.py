...
class BioToolsFunctionInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ("position", "operations_display", "inputs_display", "outputs_display", "note")
    fields = ("position", "operations_display", "inputs_display", "outputs_display", "note")
    can_delete = False
    ...
    def inputs_display(self, obj):
        return "; ".join(
            inp.get("data", {}).get("term", "") for inp in obj.inputs if inp.get("data")
        ) or "—"
    inputs_display.short_description = "Input data"

    def outputs_display(self, obj):
        return "; ".join(
            out.get("data", {}).get("term", "") for out in obj.outputs if out.get("data")
        ) or "—"
...
class BioToolsRecordAdmin(admin.ModelAdmin):
    list_display = (
        "biotools_id", "name", "submission_link",
        "last_synced_at", "sync_status", "license",
    )
    list_filter = ("maturity", "cost")
    search_fields = ("biotools_id", "name", "description")
    readonly_fields = (
        "id", "submission", "biotools_id", "name", "description", "homepage",
        "version", "license", "maturity", "cost",
        "tool_type", "operating_system",
        "edam_topic_uris", "publications", "documentation", "download", "links",
