...
class PublicationsField(forms.CharField):
    ...
    def validate(self, value: str) -> None:
        ...
        invalid = [
            t for t in tokens
            if not (self.PMID_RE.match(t) or self.DOI_RE.match(t))
        ]
        if invalid:
            raise ValidationError(
                _(f"Invalid entries: {', '.join(invalid)}. "
...
class SubmissionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "date_of_entry": forms.DateInput(
                attrs={"type": "date", "class": "form-control", "id": "id_date_of_entry"}
            ),
            "submitter_first_name": forms.TextInput(attrs={"class": "form-control", "placeholder": "e.g. Ada"}),
            "submitter_last_name": forms.TextInput(attrs={"class": "form-control", "placeholder": "e.g. Lovelace"}),
            "submitter_affiliation": forms.TextInput(attrs={"class": "form-control", "placeholder": "e.g. Forschungszentrum Jülich"}),
            ...
            "year_established": forms.NumberInput(
                attrs={"class": "form-control", "min": 1900, "max": 2100, "placeholder": "YYYY"}
            ),
            ...
            "associated_partner_note": forms.Textarea(
                attrs={"class": "form-control", "rows": 3, "id": "id_associated_partner_note"}
            ),
            ...
            "terms_of_use_url": forms.URLInput(attrs={"class": "form-control", "placeholder": "https://"}),
            ...
            "biotools_url": forms.URLInput(attrs={"class": "form-control", "placeholder": "https://bio.tools/..."}),
            "fairsharing_url": forms.URLInput(attrs={"class": "form-control", "placeholder": "https://fairsharing.org/..."}),
            "other_registry_url": forms.URLInput(attrs={"class": "form-control", "placeholder": "https://"}),

            # Section E
            "kpi_monitoring": forms.Select(attrs={"class": "form-select"}, choices=KpiMonitoring.choices),
            "kpi_start_year": forms.TextInput(attrs={"class": "form-control", "placeholder": "YYYY"}),

            # Section F
            "keywords_uncited": forms.Textarea(attrs={"class": "form-control", "rows": 2}),
            "keywords_seo": forms.Textarea(attrs={"class": "form-control", "rows": 2}),
            "outreach_consent": forms.RadioSelect(choices=[(True, "Yes"), (False, "No")]),
            "survey_participation": forms.RadioSelect(choices=[(True, "Yes"), (False, "No")]),
            "comments": forms.Textarea(attrs={"class": "form-control", "rows": 4}),

            # Section G
            "data_protection_consent": forms.CheckboxInput(attrs={"class": "form-check-input"}),
        }
        labels = {
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.fields["service_categories"].queryset = ServiceCategory.objects.filter(is_active=True)
        self.fields["service_center"].queryset = ServiceCenter.objects.filter(is_active=True)
        self.fields["responsible_pis"].queryset = PrincipalInvestigator.objects.filter(is_active=True)

        # EDAM term querysets — only active, non-obsolete terms per branch
        from apps.edam.models import EdamTerm
        self.fields["edam_topics"].queryset = (
            EdamTerm.objects.filter(branch="topic", is_obsolete=False).order_by("label")
        )
        self.fields["edam_operations"].queryset = (
            EdamTerm.objects.filter(branch="operation", is_obsolete=False).order_by("label")
        )

        # Pre-fill email confirmation from instance
        if self.instance.pk:
            self.fields["internal_contact_email_confirm"].initial = self.instance.internal_contact_email
    ...
    def clean(self) -> dict:
        ...
        if responsible_pis:
            ...
            if has_associated and not cleaned.get("associated_partner_note", "").strip():
                self.add_error(
                    "associated_partner_note",
                    _("Please provide the name and affiliation of the associated partner."),
                )
        ...
        kpi_start_year = cleaned.get("kpi_start_year", "").strip() if cleaned.get("kpi_start_year") else ""
